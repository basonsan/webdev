...
<? if($cnt): ?>
<!-- results section -->
<section class="s-results section section-bg js-section <?= ($data['TYPE'] == 50) ? 's-main' : ''?>" id="results">
    ...
    <? if($USER->IsAdmin()): ?>
        ...
        <a href='/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=60&type=content&lang=ru&ID=0' class="fix-block" target="_blank" style='top:250px'>Добавить кейс</a>
    <? endif; ?>
    <div class="container">
    <?if($data['TYPE'] == 50):?>
        ...
        <div class="row justify-content-center results__items item-items" style='position:relative;'>
            <? foreach ($data['ELEMENTS'] as $element): ?>
                ...
                <div class="col-lg-4 col-sm-6 col-12 item-item">
                    ...
                    <? if($USER->IsAdmin()): ?>
                        <a href='/bitrix/admin/iblock_element_edit.php?IBLOCK_ID=60&type=content&lang=ru&ID=<?=$element['ID']?>' class="fix-block fix-block-content" target="_blank">Редакт.</a>
                    <? endif; ?>
                    <a href="/case/<?=$element['CODE']?>/" class="item-inner item-inner_border" >
